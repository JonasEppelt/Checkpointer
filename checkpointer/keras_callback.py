...
class KerasCheckpointerCallback(BackupAndRestore):
    ...
    def __init__(self, local_checkpoint_file, save_freq="epoch", **checkpointer_kwargs) -> None:
        ...
        self.checkpointer = Checkpointer(
            ...
            checkpoint_function=lambda path, model: shutil.make_archive(
                f"{local_checkpoint_file}/checkpoint", "zip", local_checkpoint_file),  # needing to zip, since keras creates a whole dir structure
            # needing to unzip, since keras creates a whole dir structure
            restore_function=lambda path: shutil.unpack_archive(self.zip_file, local_checkpoint_file),
            **checkpointer_kwargs
        )
